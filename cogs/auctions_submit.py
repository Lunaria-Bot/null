...
def detect_rarity(embed: discord.Embed) -> str:
    text = (embed.title or "") + " " + (embed.description or "")
    for emoji, rarity in RARITY_EMOJIS.items():
        if emoji in text:
            return rarity
    return "common"
    ...
    class AuctionSetupView(discord.ui.View):
        ...
        async def submit(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            try:
                ...
                rarity = detect_rarity(self.card_embed)

                async with core.pg_pool.acquire() as conn:
                    row = await conn.fetchrow(
                        "INSERT INTO submissions(user_id, card, currency, rate, queue, status, rarity) "
                        "VALUES($1,$2::jsonb,$3,$4,$5,'submitted',$6) RETURNING id",
                        self.user.id,
